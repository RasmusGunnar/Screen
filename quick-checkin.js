...
const elements = {
  ...
  personal: document.getElementById('qc-personal'),
...
function readPersonalEmployeeId() {
  try {
    const params = new URLSearchParams(window.location.search);
    const paramId =
      params.get('employee') || params.get('emp') || params.get('id') || params.get('medarbejder');
    if (paramId) return paramId;
  } catch (error) {
    console.warn('Kunne ikke læse URL-parametre', error);
  }
  return null;
}
...
async function initQuickCheckin() {
  ...
  personalEmployeeId = readPersonalEmployeeId();
  setupEvents();
...
function renderEmployees() {
  ...
  const controls = elements.search?.closest('.qc-controls');

  if (personalEmployeeId) {
    const employee = state.employees.find((emp) => emp.id === personalEmployeeId);
    if (controls) controls.classList.add('hidden');
    elements.list.classList.add('hidden');
    elements.counter.textContent = employee
      ? 'Personlig side'
      : 'Medarbejderen blev ikke fundet';
    renderPersonalEmployee(employee);
    return;
  }

  if (controls) controls.classList.remove('hidden');
  elements.list.classList.remove('hidden');
  if (elements.personal) elements.personal.classList.add('hidden');
...
function renderPersonalEmployee(employee) {
  if (!elements.personal) return;
  elements.personal.classList.remove('hidden');
  elements.personal.innerHTML = '';
  if (!employee) {
    elements.personal.innerHTML = '<p class="qc-empty">Kunne ikke finde medarbejderen. Kontroller linket.</p>';
    return;
  }
  const card = buildEmployeeCard(employee);
  elements.personal.appendChild(card);
}
