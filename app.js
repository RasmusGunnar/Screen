...
function loadState() {
  try {
    ...
    if (raw) {
      ...
      parsed.policyAcknowledgements = parsed.policyAcknowledgements || {};
      return parsed;
    }
  } catch (error) {
...
function renderSlides() {
  elements.slides.forEach((slide, index) => {
    ...
    slide.setAttribute('aria-label', `${data.headline} – ${data.description}`);
    if (data.image) {
      slide.style.setProperty('--slide-image', `url("${data.image}")`);
    } else {
  ...
  reader.onload = (e) => {
    try {
      ...
      state = {
        ...
        logs: imported.logs || [],
        policyAcknowledgements: imported.policyAcknowledgements || {},
      };
