...
const LOCAL_STATE_KEY = 'subra-local-state';
...
function hydrateFromLocalSnapshot() {
  ...
  try {
    ...
    if (parsed && typeof parsed === 'object') {
      state = ensureStateDefaults(parsed);
      lastSyncedAt = state.updatedAt || null;
    }
  } catch (error) {
    console.warn('Kunne ikke indlæse lokal state', error);
  }
}
...
function saveLocalSnapshot() {
  if (typeof localStorage === 'undefined') return;
  try {
    localStorage.setItem(LOCAL_STATE_KEY, JSON.stringify(serializeForStorage(state)));
  } catch (error) {
    console.warn('Kunne ikke gemme lokal state', error);
  }
}
...
async function handleSlideUploadChange(event) {
  ...
  try {
    ...
    if (dataAdapter?.uploadSlide) {
      const uploaded = await dataAdapter.uploadSlide(file, () => {});
      slide.image = uploaded.downloadURL || uploaded.imageUrl || (await readFileAsDataUrl(file));
      slide.storagePath = uploaded.storagePath || null;
    } else {
    ...
    renderScreensaverAdmin();
    commitState();
    if (previousPath && previousPath !== slide.storagePath && dataAdapter?.deleteSlide) {
      void dataAdapter.deleteSlide(previousPath);
    }
    appendSyncOutput('Opdaterede pauseskærmsbilledet og gemte det tilgængeligt for forsiden.');
  } catch (error) {
